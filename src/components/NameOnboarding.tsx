...
const NameOnboarding = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // updateProfile sets profile state which triggers Index to show next screen.
      // Safety: if after 3s we're still here, force-set via a second attempt
      setTimeout(() => {
        setSaving(false);
      }, 3000);
    } catch (e) {
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-6">
        ...
        >
          ...
          <motion.button
            ...
            disabled={saving}
            className="w-full py-5 rounded-3xl bg-gradient-to-r from-kid-orange to-kid-yellow text-white font-extrabold text-xl shadow-2xl hover:shadow-[0_0_40px_hsl(var(--kid-orange)/0.5)] transition-all disabled:opacity-50 active:scale-95"

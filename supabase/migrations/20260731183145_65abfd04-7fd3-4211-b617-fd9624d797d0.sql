...
AS $function$
  ...
  BEGIN
    ...
    -- Resolve child: use provided one when it belongs to the user,
    -- otherwise fall back to the first child, creating one if needed.
    ...
    SELECT * INTO _row
    FROM public.usage
    WHERE user_id = _uid AND crianca_id = _child AND date = _today
    FOR UPDATE;
